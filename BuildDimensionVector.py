    ...
    for token in tokens_of_sentence:
        ...
        try:
            ...
            for iteration in range(1,right_threshold+1):
                ...
                required_key=('righr_'+tokens_of_sentence[required_index]).encode('utf-8')
                try:
            ...
            for iteration in range(-left_threshold,0):
                required_index=token_index+iteration
                required_key=('left_'+tokens_of_sentence[required_index]).encode('utf-8')
                try:
                    window_dictionary[token][required_key]+=1
                except:
                    temp_dict={required_key:1}
...
window_dictionary=build_window_dictionary(dimension_dict,50,file_path)
