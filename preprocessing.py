...
#note labels Good 0 Bad 1 Dialogue 2
...
def load_data(foldername):
    ...
    for file in listdir(foldername):
        with open(join(foldername,file)) as fp:
            lines=fp.readlines()
            #print file
            #print len(lines)
            #print type(lines)
            curr_qid=''
            curr_qid, ext = splitext(file)
            #print "curr qid is ---- ",curr_qid
            curr_question=''
            curr_label=''
            ...
            for line in lines:
                if line.startswith('Question!~'):
                    #print 'startline'
                    continue
                elif line.startswith('QSUBJECTBODY!~'):
                    #print 'uestion'
                    #print line.split('!~')[1]
                    curr_question=line.split('!~')[1].replace('\n','').split(' ')
                    #print curr_question
                elif line.startswith('Comment!~'):
                    #print 'label'
                    #print line.split('!~')[1].split('=')[1]
                    curr_label=line.split('!~')[1].split('=')[1]
                    if curr_label=='Dialogue':
                        curr_label=2
                    elif curr_label=='Good':
                        curr_label=0
                    else:
                        curr_label=1
                else :
                    #print 'answer'
                    #print line.split('!~')[1]
                    #curr_question=curr_question.split(' ')
                    qids.append(curr_qid)
                    labels.append(curr_label)
                    questions.append(curr_question)
                    curr_answer=line.split('!~')[1].replace('\n','').split(' ')
                    answers.append(curr_answer)
    print len(qids),len(questions),len(answers),len(labels)
    print 'unique questions', len(set(qids))
...
if __name__ == '__main__':
  ...
  for train in train_files:
    ...
    # all_fname = train
    #all_fname = "/tmp/ansrank-merged.txt"
    ...
    qids, questions, answers, labels = load_data(train)
    #print questions
    ...
    with open('tempwordfs', 'wb') as f:
        pickle.dump(word2dfs, f)
    ...
    for fname in [train]:
      ...
      #without stop words sending df_overlap float and overlap float value
      overlap_feats = compute_overlap_features(questions, answers, stoplist=None, word2df=word2dfs)
      #with stop words sending df_overlap float and overlap float value
      ...
      #np.save(os.path.join(outdir, '{}.qids.txt'.format(basename)), qids)
