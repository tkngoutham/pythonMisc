        ...
        for line in fp:
            ...
            for token in tokens:
                ...
                if(len(word_tag)<2):
                    ...
                    Current_Tag=word_tag[1].strip()
                    try:
                        if Word_Dictionary[Current_Word]<5:
                            Current_Word='_RARE_'
                    except:
                        continue
...
def pi(position,curr_pos_tag,smoothed_sentence):
    if position==0:
        ...
        for index in range(0,len(Taggable_Tags)):
            probability_buffer[index]=pi(position-1,Taggable_Tags[index],smoothed_sentence)*bigram_transition(curr_pos_tag,Taggable_Tags[index])*emission_prob
        backtracking_Dictionary[position][curr_pos_tag]=Taggable_Tags[probability_buffer.index(max(probability_buffer))]
...
def viterbi(words_in_sent):
    ...
    for Tag in Taggable_Tags:
        probability_buffer[Taggable_Tags.index(Tag)]=pi(position,Tag,words_in_sent)
...
def initiate_viterbi(sentence_file_path):
    ...
    with open(sentence_file_path,'r') as fp:
        for line in fp:
            ...
            smoothed_sentence=''
            output_file=open('/home/gowtham/eclipse/A2/results/hin_result','a')
...
#training_data_path=r'C:\Users\innoveta\Desktop\PGMs\tel6.txt'
training_data_path='/home/gowtham/eclipse/A2/TrainingData/hin_tagged'
smoothed_training_data_path='/home/gowtham/eclipse/A2/TrainingData/hin_tagged_smoothed'
